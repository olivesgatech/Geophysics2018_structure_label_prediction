function [v,usediters] = projfunc( s, k1, k2, nn, eta)
...
%
% EDIT: YAZEED ALAUDAH
% the algorithm gets stuck sometimes, and never converges. 
% to solve this, I added a check in the while loop, if the
% sparsity becomes more than eta (input), then the loop stops.
% this shouldn't make a big difference, since the negative values it 
% keeps running because are usually very small (like 1e-9 or smaller)
...
while 1,
    ...
    sparsity = 1-(sum(v~=0)/numel(v));
    if sparsity > eta
        zerocoeff = find(v<=0);
        v(zerocoeff) = 0;
%         disp('Warning: projfun.m did not converge.');
        break;
    end
